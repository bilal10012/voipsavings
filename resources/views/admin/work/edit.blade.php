            ...
            <div class="col-md-6">
                <div class="block">
                    <div class="block-content block-content-full">
                        <label>Our Work Image</label>
                        <form class="dropzone" id="primaryImage" action="{{route('admin.our-work.upload', $image->id)}}" method="POST">@csrf</form>
                        <small>To update this image, click inside the zone or drop the new image inside the zone.</small>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <form method="POST" class="p-20" action="{{route('admin.our-work.update', $image->id)}}">
                    @csrf
                    @method('PUT')
