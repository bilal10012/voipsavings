...
@section('content')
    <div class="content">
        <h2 class="content-heading">testimonial <a href="{{route('admin.testimonial.create')}}" class="btn btn-alt-primary pull-right"><i class="fa fa-plus-circle"></i> Add new testimonial</a></h2>
        <div class="block">
            ...
            <table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                    ...
                    <tbody>
                    @forelse($testimonial as $key => $service)
                    <tr>
                        ...
                        <td class="text-center">
                            <div class="btn-group">
                                ...
                                <!-- <a href="{{route('admin.service.show', $service->id)}}" class="btn btn-sm btn-secondary" data-toggle="tooltip" title="View Product">
                                    <i class="fa fa-eye text-primary"></i>
                                </a> -->

                                
                                @endif
                                <a href="{{route('admin.testimonial.feature', $service->id)}}" class="btn btn-sm btn-secondary" data-toggle="tooltip" title="{{($service->is_featured)? 'Unmark as Featured': 'Mark as Featured'}}">
                                ...
                                <a href="{{route('admin.testimonial.edit', $service->id)}}" class="btn btn-sm btn-secondary" data-toggle="tooltip" title="Edit Product">
                        ...
                        <form id="delete-{{$service->id}}" action="{{ route('admin.testimonial.destroy',  $service->id) }}" style="display:none" method="POST">
                         {{ method_field('DELETE') }}
                         {{ csrf_field() }}
                    </form>
                    </tr>
